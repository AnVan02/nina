    ...
    <form method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <?php if(isset($config[$com][$type][$dfgallery][$val]['soluongtu']) && $config[$com][$type][$dfgallery][$val]['soluongtu'] == true) { ?>
                        <div class="form-group col-md-6">
                            <label class="d-block" for="soluongtu">Số lượng từ:</label>
                            <div class="input-group">
                                <input type="number" class="form-control format-price" name="data[soluongtu]" id="soluongtu" placeholder="Số lượng từ" value="<?=@$item['soluongtu']?>">
                            </div>
                        </div>
                    <?php } ?>
                    <?php if(isset($config[$com][$type][$dfgallery][$val]['soluongden']) && $config[$com][$type][$dfgallery][$val]['soluongden'] == true) { ?>
                        <div class="form-group col-md-6">
                            <label class="d-block" for="soluongden">Số lượng đến:</label>
                            <div class="input-group">
                                <input type="number" class="form-control format-price" name="data[soluongden]" id="soluongden" placeholder="Số lượng đến" value="<?=@$item['soluongden']?>">
                            </div>
                        </div>
                    <?php } ?>
                    <?php if(isset($config[$com][$type][$dfgallery][$val]['phantram']) && $config[$com][$type][$dfgallery][$val]['phantram'] == true) { ?>
                        <div class="form-group col-md-6">
                            <label class="d-block" for="phantram">Phần trăm:</label>
                            <div class="input-group">
                                <input type="number" class="form-control format-price" name="data[phantram]" id="phantram" placeholder="Phần trăm" value="<?=@$item['phantram']?>">
                            </div>
                        </div>
                    <?php } ?>
                    <?php if(isset($config[$com][$type][$dfgallery][$val]['giagiam']) && $config[$com][$type][$dfgallery][$val]['giagiam'] == true) { ?>
                        <div class="form-group col-md-6">
                            <label class="d-block" for="giagiam">Giá giảm:</label>
                            <div class="input-group">
                                <input type="text" class="form-control format-price gia_ban" name="data[giagiam]" id="giagiam" placeholder="Giá giảm" value="<?=@$item['giagiam']?>">
                    ...
                    <?php } ?>
                    <?php if(isset($config[$com][$type][$dfgallery][$val]['tuychon']) && $config[$com][$type][$dfgallery][$val]['tuychon'] == true) { ?>
                    <div class="form-group col-sm-6">
                        <label for="loaihienthi">Giảm theo:</label>
                        <select class="form-control" name="data[tuychon]" id="tuychon">
                            <option <?=(isset($item['tuychon']) && $item['tuychon'] == 0)?"selected":""?> value="0">Phần trăm</option>
                            <option <?=(isset($item['tuychon']) && $item['tuychon'] == 1)?"selected":""?> value="1">Giá giảm</option>
                        </select>
                    </div>
                    <?php }?>
                    </div>
