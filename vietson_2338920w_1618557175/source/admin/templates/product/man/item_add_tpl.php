		                            ...
		                            <?php foreach($config['website']['lang'] as $k => $v) { ?>
		                                <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
		                                    ...
		                                    <?php if(isset($config['product'][$type]['noidung']) && $config['product'][$type]['noidung'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="noidung<?=$k?>">Thông tin chi tiết (<?=$k?>):</label>
		                                            <textarea class="form-control for-seo <?=(isset($config['product'][$type]['noidung_cke']) && $config['product'][$type]['noidung_cke'] == true)?'form-control-ckeditor':''?>" name="data[noidung<?=$k?>]" id="noidung<?=$k?>" rows="5" placeholder="Thông tin chi tiết (<?=$k?>)"><?=htmlspecialchars_decode(@$item['noidung'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                    <?php if(isset($config['product'][$type]['cauhinh']) && $config['product'][$type]['cauhinh'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="cauhinh<?=$k?>">Cấu hình chi tiết (<?=$k?>):</label>
		                                            <textarea class="form-control for-seo <?=(isset($config['product'][$type]['cauhinh_cke']) && $config['product'][$type]['cauhinh_cke'] == true)?'form-control-ckeditor':''?>" name="data[cauhinh<?=$k?>]" id="cauhinh<?=$k?>" rows="5" placeholder="Cấu hình chi tiết (<?=$k?>)"><?=htmlspecialchars_decode(@$item['cauhinh'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                    <?php if(isset($config['product'][$type]['luuy']) && $config['product'][$type]['luuy'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="luuy<?=$k?>">Lưu ý (<?=$k?>):</label>
		                                            <textarea class="form-control for-seo <?=(isset($config['product'][$type]['luuy_cke']) && $config['product'][$type]['luuy_cke'] == true)?'form-control-ckeditor':''?>" name="data[luuy<?=$k?>]" id="luuy<?=$k?>" rows="5" placeholder="Lưu ý (<?=$k?>)"><?=htmlspecialchars_decode(@$item['luuy'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                </div>
		                            <?php } ?>
				            	...
				            	<?php if(isset($config['product'][$type]['dropdown']) && $config['product'][$type]['dropdown'] == true) { ?>
				            		<?php if(isset($config['product'][$type]['list']) && $config['product'][$type]['list'] == true) { ?>
						                <div class="form-group col-xl-6 col-sm-4">
						                    <label class="d-block" for="id_list">Hãng sản xuất:</label>
											<?=$func->get_ajax_category2('product', 'list', $type)?>
						                </div>
						            <?php } ?>
						            <?php if(isset($config['product'][$type]['cat']) && $config['product'][$type]['cat'] == true) { ?>
						                <div class="form-group col-xl-6 col-sm-4">
						                    <label class="d-block" for="id_cat">Loại sản phẩm:</label>
											<?=$func->get_ajax_category3('product', 'cat', $type, $_GET['id'])?>
						                </div>
						            <?php } ?>
	    ...
	    <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control form-control-mini d-inline-block align-middle" min="0" name="data[stt]" id="stt" placeholder="Số thứ tự" value="<?=isset($item['stt']) ? $item['stt'] : $func->updatestt('product',$_GET['type'])?>">
                </div>
                <div class="row">
	                	<div class="form-group col-md-4">
	                        <label class="d-block" for="trangthai">Trạng thái:</label>
	                        <input type="text" class="form-control" name="data[trangthai]" id="trangthai" placeholder="Trạng thái" value="<?=@$item['trangthai']?>">
	                    </div>
	                    <div class="form-group col-md-4">
	                        <label class="d-block" for="diemcongty">Điểm của công ty:</label>
	                        <input type="number" class="form-control" name="data[diemcongty]" id="diemcongty" placeholder="Điểm của công ty" value="<?=@$item['diemcongty']?>">
	                    </div>
	                    <div class="form-group col-md-4">
	                        <label class="d-block" for="diemhang">Điểm của hãng:</label>
	                        <input type="number" class="form-control" name="data[diemhang]" id="diemhang" placeholder="Điểm của hãng" value="<?=@$item['diemhang']?>">
	                    </div>
	                	...
	                	<div class="form-group col-md-4">
	                        ...
	                        <input type="number" class="form-control" name="data[soluong]" id="soluong" placeholder="Số lượng" value="<?=@$item['soluong']?>">
	                    </div>
				    ...
				    <?php if(isset($config['product'][$type]['giasi']) && $config['product'][$type]['giasi'] == true) { ?>
				    	<div class="form-group col-md-4">
	                        <label class="d-block" for="giasi">Giá đại lý:</label>
	                        <div class="input-group">
	                        	<input type="text" class="form-control format-price gia_moi" name="data[giasi]" id="giasi" placeholder="Giá đại lý" value="<?=@$item['giasi']?>">
	                        	<div class="input-group-append">
	                        		<div class="input-group-text"><strong>VNĐ</strong></div>
	                        	</div>
	                        </div>
        ...
        <div class="show_propeties">
	        ...
	        <div class="card card-primary card-outline text-sm">
	            ...
	            <div class="card-body">
	            	<div class="row">
	            	<?php
	            	$tempstt = $d->rawQueryOne("select id_thuoctinh from #_product_cat where id = ? and type = ? limit 0,1",array($_GET['id_cat'],$type));
					$arr_tt = explode(',', $tempstt['id_thuoctinh']);

					$thuoctinh = $d->rawQuery("select tenvi, id from #_product_brand where  type = ? order by stt,id desc",array($type));
	            	for($i=0;$i<count($thuoctinh);$i++){
	            		$giatri = $d->rawQueryOne("select * from #_product_properties where id_properties = ? and id_product = ? and type = ? limit 0,1",array($thuoctinh[$i]['id'],$item['id'],$type));
	            		if(!in_array($thuoctinh[$i]['id'],$arr_tt)) continue;
	            	?>
	                <div class="form-group col-md-4">
	                    <label class="d-block" for="masp"><?=$thuoctinh[$i]['tenvi']?>:</label>
	                    <input type="hidden" class="form-control" name="data2[id_cat][]" id="id_cat" value="<?=@$_GET['id_cat']?>">
