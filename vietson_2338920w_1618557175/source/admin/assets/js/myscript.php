<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		/* Ajax category */
		$('body').on('change','.select-category2', function(){
			var id = $(this).val();
			var child = $(this).data("child");
			var level = parseInt($(this).data('level'));
			var table = $(this).data('table');
		...
		/* Ajax category */
		$('body').on('change','.select-category3', function(){
			var id = $(this).val();
			var id_pro=$(this).data('pro');
			$.ajax({
				url: 'ajax/ajax_category3.php',
				type: 'POST',
				data: {id:id, id_pro:id_pro},
				success: function(result)
					{
						$(".show_propeties").html(result);
					}
			});
		});
