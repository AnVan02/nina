<?php
	...
	function save_item()
	{
		...
		if($data)
		{
			...
			$data['giasi'] = (isset($data['giasi']) && $data['giasi'] != '') ? str_replace(",","",$data['giasi']) : 0;
		...
		if($id && $act!='save_copy')
		{
			...
			if($d->update('product',$data))
			{
				...
				if(isset($_POST['data2']) && $_POST['data2'] != '')
				{
					...
					for($i=0;$i<count($_POST['data2']['id_cat']);$i++){
						$datapro['id_cat'] = $_POST['data2']['id_cat'][$i];
						$datapro['id_properties'] = $_POST['data2']['id_properties'][$i];
						$datapro['tenvi'] = $_POST['data2']['tenvi'][$i];
						$datapro['giatri'] = $_POST['data2']['giatri'][$i];
						$datapro['id_product'] = $id;
						$datapro['type'] = $type;
						if($_POST['data2']['id_giatri'][$i]!=''){
							$d->where('id', $_POST['data2']['id_giatri'][$i]);
							$d->update('product_properties',$datapro);
						}else{
							$d->insert('product_properties',$datapro);
						}
	...
	function delete_item()
	{
		...
		if($id)
		{
			...
			$d->rawQuery("delete from #_product_properties where id_product = ? and type = ?",array($id,$type));
	...
	function save_cat()
	{
		...
		/* Post dữ liệu */
		$data = (isset($_POST['data'])) ? $_POST['data'] : null;
		if(isset($_POST['list_group']) && ($_POST['list_group'] != '')) $data['id_list'] = implode(",", $_POST['list_group']);
				else $data['id_list'] = "";
		if(isset($_POST['properties_group']) && ($_POST['properties_group'] != '')) $data['id_thuoctinh'] = implode(",", $_POST['properties_group']);
				else $data['id_thuoctinh'] = "";
		if($data)
		{
			foreach($data as $column => $value)
			{
