<?php
	...
	function save_photo()
	{
		...
		if($data)
		{
			...
			$data['giagiam'] = (isset($data['giagiam']) && $data['giagiam'] != '') ? str_replace(",","",$data['giagiam']) : 0;
		}
			...
			if($_GET['val']=='so-luong')
			{
				...
				$success = 0;
					...
					for($i=0;$i<count($dataMultiTemp);$i++)
					{
						...
						$dataMulti['giagiam'] = (isset($dataMultiTemp[$i]['giagiam']) && $dataMultiTemp[$i]['giagiam'] != '') ? str_replace(",","",$dataMultiTemp[$i]['giagiam']) : 0;
						$dataMulti['com'] = $com;
						$dataMulti['type'] = $type;
						$dataMulti['kind'] = $kind;
						$dataMulti['val'] = $val;
