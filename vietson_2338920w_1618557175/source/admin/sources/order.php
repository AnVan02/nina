	...
	function save_item()
	{
		...
		if($id)
		{
			...
			if($d->update('order',$data)) {
				if($_POST['data']['tinhtrang']==4){
					$detail_order = $d->rawQuery("select * from table_order_detail where id_order = ?  order by id desc",array($id));
					$order = $d->rawQueryOne("select * from #_order where id = ? limit 0,1",array($id));
					$row_user = $d->rawQueryOne("select * from #_member where id = ? limit 0,1",array($order['id_user']));
	
					$tongdiemhang=0;
					$tongdiemcongty=0;
					foreach ($detail_order as $key => $value) {
						$proorder = $d->rawQueryOne("select * from #_product where id = ? limit 0,1",array($value['id_product']));
					...
					$data_diem['diemhang'] = $row_user['diemhang']+$tongdiemhang;
					$data_diem['diemcongty'] = $row_user['diemcongty']+$tongdiemcongty;
		        	$d->where('id', $order['id_user']);
		        	$d->update('member',$data_diem);
				}
