<?php
	...
	$quantity = (isset($_POST['quantity']) && $_POST['quantity'] > 0) ? htmlspecialchars($_POST['quantity']) : 0;
	...
	if($cmd == 'add-cart' && $id > 0)
	{
		...
		$numb=$cart->maxnumber($id);
		$valn=$cart->shownumber($id);
		$data = array('max' => $max, 'numb' => $numb, 'valn' => $valn);
	...
	else if($cmd == 'update-cart' && $id > 0 && $code != '')
	{
		...
		$giasi = $func->format_money($proinfo['giasi']*$quantity);
		...
		if(isset($_SESSION[$login_member])){
			$giamoi = $giasi;
			$saleuser = $cart->get_sale_price($id,$quantity);
			$pricenew=$func->format_money($cart->total_get_price($id,$quantity));
		}else{
			$giamoi = $giamoi;
		}
		$numb=$cart->maxnumber($id);
		$valn=$cart->shownumber($id);
		$data = array('gia' => $gia, 'giamoi' => $giamoi, 'giasi' => $giasi, 'pricenew' => $pricenew, 'saleuser' => $saleuser, 'temp' => $temp, 'tempText' => $tempText, 'total' => $total, 'totalText' => $totalText, 'numb' => $numb, 'valn' => $valn);
		    ...
		    <div class="wrap-cart">
		        <div class="top-cart">
		            ...
		            <div class="modal-footer d-flex align-items-center justify-content-between">
		                <a href="javascript:void(0)" class="buymore-cart text-decoration-none" title="<?=tieptucmuahang?>" data-dismiss="modal" aria-label="Close">
