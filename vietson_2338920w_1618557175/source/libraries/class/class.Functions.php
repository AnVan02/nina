<?php
	class Functions
	{
		...
		public function checkwarter($check=false)
		{	
			if($check==true){
				$war='watermark/product';
			}else{
				$war='thumbs';
			}
			return $war;
		...
		public function updatestt($table='', $type='')
		{	

			global $login_member;

			$row = $this->d->rawQueryOne("select MAX(stt) as max from #_".$table." where type = ?",array($type));
			return $row['max']+1;
		}

		public function priceold($id=0,$dv='đ')
		{
			global $login_member;
			$row = $this->d->rawQueryOne("select id,gia,giamoi,giasi from #_product where id = ?",array($id));
			if(isset($_SESSION[$login_member]) && $row['giasi']>0){
				$show_priceold='';
			}elseif($row['giamoi']>0 && $row['gia']>0){
				$show_priceold=' - <i>'.number_format($row['gia'],0,",",",").$dv.'</i>';
			}
			else{
				$show_priceold='';
			}
			return $show_priceold;
		}
		public function pricenew($id=0,$dv='đ')
		{
			global $login_member;
			$row = $this->d->rawQueryOne("select id,gia,giamoi,giasi from #_product where id = ?",array($id));

			if(isset($_SESSION[$login_member]) && $row['giasi']>0){
				$show_pricenew=number_format($row['giasi'],0,",",",").$dv;
			}elseif($row['giamoi']>0){
			$show_pricenew=number_format($row['giamoi'],0,",",",").$dv;
			}else{
				if($row['gia']==0){
				$show_pricenew='<a href="lien-he"> '.lienhe.'</a>';
				}else{
					$show_pricenew=number_format($row['gia'],0,",",",").''.$dv;
				}
			}
			
			return $show_pricenew;
		}

		public function sale($id=0){
			$row = $this->d->rawQueryOne("select id,gia,giamoi from #_product where id = ?",array($id));
			if($row['giamoi']>0 && $row['gia']>0){
				$sale=100-(round(($row['giamoi']/$row['gia'])*100));
				$show_sale='<span class="sale"> -'.$sale.'%</span>';
		...
		/* Get list */
		public function get_list($id=0, $element='', $table='', $type='')
		{
			if($id)
			{
				$temps = $this->d->rawQueryOne("select id_list from #_".$table." where id = ? and type = ? limit 0,1",array($id,$type));
				$arr_tags = explode(',', $temps['id_list']);
				
				for($i=0;$i<count($arr_tags);$i++) $temp[$i]=$arr_tags[$i];
			}

			$row_tags = $this->d->rawQuery("select tenvi, id from #_product_list where type = ? order by stt,id desc",array($type));
		...
		/* Get list */
		public function get_user_group($id=0, $element='', $table='', $type='')
		{
			if($id)
			{
				$temps = $this->d->rawQueryOne("select id_user from #_".$table." where id = ? and type = ? limit 0,1",array($id,$type));
				$arr_tags = explode(',', $temps['id_user']);
				
				for($i=0;$i<count($arr_tags);$i++) $temp[$i]=$arr_tags[$i];
			}

			$row_tags = $this->d->rawQuery("select username, id from #_member where hienthi = 1 order by stt,id desc");
			...
			for($i=0;$i<count($row_tags);$i++)
			{
				...
				$str .= '<option value="'.$row_tags[$i]["id"].'" '.$selected.' /> '.$row_tags[$i]["username"].'</option>';
			}
			$str .= '</select>';
		...
		/* Get Properties */
		public function get_properties($id=0, $element='', $table='', $type='')
		{
			if($id)
			{
				$temps = $this->d->rawQueryOne("select id_thuoctinh from #_".$table." where id = ? and type = ? limit 0,1",array($id,$type));
				$arr_tags = explode(',', $temps['id_thuoctinh']);
				
				for($i=0;$i<count($arr_tags);$i++) $temp[$i]=$arr_tags[$i];
			}

			$row_tags = $this->d->rawQuery("select tenvi, id from #_product_brand where type = ? order by stt,id desc",array($type));
	    ...
	    /* Get category by ajax */
		public function get_ajax_category2($table='', $level='', $type='', $title_select='Chọn danh mục', $class_select='select-category2')
	    {
	    	$where = '';
	    	$params = array($type);
	    	$id_parent = 'id_'.$level;
	    	...
	    	$sed="select tenvi, id from #_".$table."_".$level." where type = ? ".$where." order by stt,id desc";
	        $rows = $this->d->rawQuery("select tenvi, id from #_".$table."_".$level." where type = ? ".$where." order by stt,id desc",$params);
	     ...
	     /* Get category by ajax */
		public function get_ajax_category3($table='', $level='', $type='', $id_pro=0, $title_select='Chọn danh mục', $class_select='select-category3')
	    {
	    	$where = '';
	    	$params = array($type);
	    	$id_parent = 'id_'.$level;
	        ...
	        $str = '<select id="'.$id_parent.'" name="data['.$id_parent.']" '.$data_level.' '.$data_type.' '.$data_table.' '.$data_child.' data-pro="'.$id_pro.'" class="form-control select2 '.$class_select.'"><option value="0">'.$title_select.'</option>';
	        foreach($rows as $v)
	        {
	            if(isset($_REQUEST[$id_parent]) && ($v["id"] == (int)$_REQUEST[$id_parent])) $selected = "selected";
	            else $selected = "";
