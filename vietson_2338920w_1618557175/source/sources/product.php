<?php  
	...
	@$idc = htmlspecialchars($cate);
	@$hang = htmlspecialchars($brand);
	@$idi = htmlspecialchars($_GET['idi']);
	@$ids = htmlspecialchars($_GET['ids']);
	@$idb = htmlspecialchars($_GET['idb']);
	//echo count($_SESSION['compare']);die();
	if(isset($_SESSION['compare']) && count($_SESSION['compare'])>0){
		$arr_comp='';
		for ($i=0; $i < count($_SESSION['compare']); $i++) { 
			$arr_comp[]=$_SESSION['compare'][$i]['productid'];
		}
		if(count($_SESSION['compare'])>0){
			$check_compare=$_SESSION['compare'][0]['cate'];
		}
	...
	{
		/* Lấy sản phẩm detail */
		$row_detail = $d->rawQueryOne("select type, id, ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang, noidung$lang, cauhinh$lang, luuy$lang, masp, luotxem, id_brand, id_mau, id_size, id_list, id_cat, id_item, id_sub, id_tags, photo, options, giakm, giamoi, gia, giasi, soluong from #_product where id = ? and type = ? and hienthi > 0 limit 0,1",array($id,$type));
		...
		$sql = "select photo, ten$lang, mota$lang, tenkhongdauvi, tenkhongdauen, giamoi, gia, giakm, id,id_cat, type from #_product where $where order by stt,id desc $limit";
	...
	{	
		$pro_hang = $d->rawQueryOne("select id, ten$lang, tenkhongdauvi, tenkhongdauen from #_product_list where tenkhongdauvi = ? and type = ? limit 0,1",array($hang,$type));
		/* Lấy cấp 2 detail */
		$pro_cat = $d->rawQueryOne("select id, id_list, ten$lang, tenkhongdauvi, tenkhongdauen, type, photo, options from #_product_cat where tenkhongdauvi = ? and type = ? limit 0,1",array($idc,$type));
		if(isset($_SESSION['compare']) && count($_SESSION['compare'])>0){
			if($check_compare!=$idc){
		...
		/* Lấy sản phẩm */
		$where = "";
		$where = "id_list = ? and id_cat = ? and type = ? and hienthi > 0";
		$params = array($pro_hang['id'],$pro_cat['id'],$type);
		...
		/* breadCrumbs */
		if(isset($title_crumb) && $title_crumb != '') $breadcr->setBreadCrumbs($com,$title_crumb);
		if($pro_hang != null) $breadcr->setBreadCrumbs($pro_hang[$sluglang],$pro_hang['ten'.$lang]);
		if($pro_cat != null) $breadcr->setBreadCrumbs($pro_cat[$sluglang],$pro_cat['ten'.$lang]);
		$breadcrumbs = $breadcr->getBreadCrumbs();
	}
	else if($idi!='')
