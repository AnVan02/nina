<?php  
    ...
    $sale = $d->rawQueryOne("select id, type, ten$lang, mota$lang, photo, ngaytao, ngaysua, options from #_static where type = ? limit 0,1",array('chuong-trinh-khuyen-mai'));

    $prohot = $d->rawQueryOne("select ten$lang, mota$lang, photo, link, hienthi from #_photo where type = ? and act = ? limit 0,1",array('san-pham','photo_static'));
    ...
    $product = $d->rawQuery("select id,ten$lang,mota$lang, tenkhongdauvi, tenkhongdauen,photo,masp from #_product where type = ? and noibat > 0 and hienthi > 0",array('san-pham'));
