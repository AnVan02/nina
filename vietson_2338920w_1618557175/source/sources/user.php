<?php
	...
	switch($action)
	{
		...
		case 'quen-mat-khau':
			...
			if(isset($_SESSION[$login_member]['active']) && $_SESSION[$login_member]['active'] == true) $func->transfer("Trang không tồn tại",$config_base, false);
			if(isset($_POST['quenmatkhau'])) quenmatkhau_user();
			break;
		...
		case 'thong-tin-ca-nhan':
			...
			$title_crumb = 'Thông tin cá nhân';
			info_user();
			break;

		case 'doi-mat-khau':
			if(!isset($_SESSION[$login_member]['active']) || !$_SESSION[$login_member]['active']) $func->transfer("Trang không tồn tại",$config_base, false);
			$template = "account/doimatkhau";
		...
		case 'tao-don-hang':
			if(!isset($_SESSION[$login_member]['active']) || !$_SESSION[$login_member]['active']) $func->transfer("Trang không tồn tại",$config_base, false);
			$template = "account/taodonhang";
			$title_crumb = 'Tạo đơn hàng';
			create_order();
			break;
		...
		case 'lich-su-don-hang':
			if(!isset($_SESSION[$login_member]['active']) || !$_SESSION[$login_member]['active']) $func->transfer("Trang không tồn tại",$config_base, false);
			$template = "account/lichsudonhang";
			$title_crumb = 'Lịch sử đơn hàng';
			history_order();
			break;
		...
		case 'trang-thai-don-hang':
			if(!isset($_SESSION[$login_member]['active']) || !$_SESSION[$login_member]['active']) $func->transfer("Trang không tồn tại",$config_base, false);
			$template = "account/trangthaidonhang";
			$title_crumb = 'Trạng thái đơn hàng';
			status_order();
			break;

		case 'thong-bao':
			if(!isset($_SESSION[$login_member]['active']) || !$_SESSION[$login_member]['active']) $func->transfer("Trang không tồn tại",$config_base, false);
			$template = "account/thongbao";
		...
		case 'diem-thuong':
			if(!isset($_SESSION[$login_member]['active']) || !$_SESSION[$login_member]['active']) $func->transfer("Trang không tồn tại",$config_base, false);
			$template = "account/diemthuong";
			$title_crumb = 'Điểm thưởng';
			point_order();
			break;


		case 'thay-doi-don-hang':
			if(!isset($_SESSION[$login_member]['active']) || !$_SESSION[$login_member]['active']) $func->transfer("Trang không tồn tại",$config_base, false);
			$template = "account/thaydoidonhang";
			$title_crumb = 'Thay đổi dơn hàng';
			edit_order();
			break;
	...
	function info_user()
	{
		...
		if($iduser)
		{
		    ...
		    if(isset($_POST['capnhatthongtin']))
		    {
		        ...
		        {
		        	$func->transfer("Cập nhật thông tin thành công",$config_base."account/thong-tin-ca-nhan");	            
		        }
		    }
		}
		else
		...
		if($iduser)
		{
		    ...
		    if(isset($_POST['capnhatthongtin']))
		    {
		        ...
		        if($password)
		        {
		            ...
		            if(!isset($row['id'])) $func->transfer("Mật khẩu cũ không chính xác","account/doi-mat-khau", false);
		            if(!$new_password || ($new_password != $new_password_confirm)) $func->transfer("Thông tin mật khẩu mới không chính xác","account/doi-mat-khau", false);
	...
	{
		global $d, $func, $row_detail, $config_base, $login_member, $lang, $get_page, $product, $sluglang, $paging;
		...
		if($iduser)
		{
			...
			if(isset($_GET['s'])){
				$tukhoa = htmlspecialchars($_GET['s']);
				$tukhoa = $func->changeTitle($tukhoa);
				$typesearch = htmlspecialchars($_GET['type']);
				if($typesearch=='name'){
					...
					$where = "type = ? and (masp LIKE ?) and hienthi > 0";
					$params = array("san-pham","%$tukhoa%");
				}
				...
				$sql = "select photo, ten$lang, mota$lang, tenkhongdauvi, tenkhongdauen, giamoi, gia, giasi, masp, giakm, id, id_cat, trangthai, type, soluong from #_product where $where order by stt,id desc $limit";
	...
	{
		global $d, $func, $row_detail, $config_base, $login_member, $lang, $get_page, $orderdr, $sluglang, $paging;
	...
	function status_order()
	{
		...
		if($iduser)
		{
			...
			if(isset($_GET['s'])){
				//echo $_GET['s'];die();
				$tukhoa = htmlspecialchars($_GET['s']);
				$where = "id_user = ? and (madonhang LIKE ?)";
				$params = array($iduser,"%$tukhoa%");
				$curPage = $get_page;
				$per_page = 20;
				$startpoint = ($curPage * $per_page) - $per_page;
				$limit = " limit ".$startpoint.",".$per_page;
	...
	{
		global $d, $func, $row_detail, $config_base, $login_member, $lang, $get_page,$template, $news, $sluglang, $paging;

		$iduser = $_SESSION[$login_member]['id'];
		if($iduser)
		{	
			if(isset($_GET['id'])){
				$row_detail = $d->rawQueryOne("select ten$lang,noidung$lang from #_news where find_in_set(".$iduser.",id_user) and id=? and type = ? limit 0,1",array($_GET['id'],'thong-bao'));
				$template="account/detail_thongbao";
			}else{
				$row_detail = $d->rawQueryOne("select ten, username, gioitinh, ngaysinh, email, dienthoai, diachi from #_member where id = ? limit 0,1",array($iduser));
				$where = "";
				$where = "find_in_set(".$iduser.",id_user) and type = ?";
				$params = array('thong-bao');
	...
	{
		global $d, $func, $row_detail, $config_base, $login_member, $lang, $get_page, $orderdr,$totalhang,$totalcongty, $sluglang, $paging;

		$iduser = $_SESSION[$login_member]['id'];
		if($iduser)
		{
			$row_detail = $d->rawQueryOne("select ten, username, gioitinh, ngaysinh, email, dienthoai, diachi, diemhang, diemcongty from #_member where id = ? limit 0,1",array($iduser));

			$orderuser = $d->rawQuery("select * from table_order where id_user = ? and tinhtrang=4  order by id desc",array($iduser));
			$totalhang=0;
            $totalcongty=0;
			for ($i=0; $i < count($orderuser); $i++) { 
				$order_point = $d->rawQuery("select * from table_order_detail where id_order = ?  order by id desc",array($orderuser[$i]['id']));

				$totalitemhang=0;
                $totalitemcongty=0;
				foreach ($order_point as $k => $val) {
					$proppoint = $d->rawQueryOne("select id, diemhang,diemcongty, ten$lang, tenkhongdauvi, tenkhongdauen,photo from #_product where id = ? limit 0,1",array($val['id_product']));
					$pointhang=$proppoint['diemhang']*$val['soluong'];
                    $totalitemhang +=$pointhang;
                    $pointconty=$proppoint['diemcongty']*$val['soluong'];
                    $totalitemcongty +=$pointconty;
				}

				$totalhang +=$totalitemhang;
            	$totalcongty +=$totalitemcongty;
			}
			$where = "";
			$where = "id_user = ? and tinhtrang=4";
			$params = array($iduser);
			$curPage = $get_page;
			$per_page = 20;
			$startpoint = ($curPage * $per_page) - $per_page;
	...
	function edit_order()
	{
		global $d, $func, $row_detail, $config_base, $login_member, $lang, $cart, $get_page, $orderdr, $sluglang, $paging;
		...
		if($iduser)
		{
			...
			if($_POST){
		        ...
		        for ($i=0; $i < count($_POST['id_product']) ; $i++) { 
		        	$tong +=$cart->total_order_end($_POST['id_product'][$i],$_POST['soluong'][$i]);
		        }
		        $data['tamtinh'] = $tong;
	        	$data['tonggia'] = $tong;
	        	$d->where('id', $_POST['id']);
		        ...
		        {
		        	for ($j=0; $j < count($_POST['id_product']) ; $j++) { 
		        		$data2['soluong'] = $_POST['soluong'][$j];
			        	$d->where('id_order', $_POST['id']);
			        	$d->where('id_product', $_POST['id_product'][$j]);
			        	$d->update('order_detail',$data2);
		        	}
		        	$func->transfer("Thay đổi đơn hàng thành công",$config_base."account/thay-doi-don-hang");	            
		        }

			}else{
				$where = "";
				$where = "id_user = ? and tinhtrang=1";
				$params = array($iduser);
				$curPage = $get_page;
				$per_page = 20;
				$startpoint = ($curPage * $per_page) - $per_page;
	...
	function login()
	{
		...
		if(isset($row['id']) && $row['id'] > 0)
		{
			if($row['password'] == $passwordMD5)
			{
				...
				$func->transfer("Đăng nhập thành công", 'account/thong-tin-ca-nhan');
			}
			else
			{
	...
	function signup()
	{
		...
		$data['tendoanhnghiep'] = (isset($_POST['tendoanhnghiep'])) ? htmlspecialchars($_POST['tendoanhnghiep']) : '';
		$data['masothue'] = (isset($_POST['masothue'])) ? htmlspecialchars($_POST['masothue']) : '';
		$data['ten'] = (isset($_POST['ten'])) ? htmlspecialchars($_POST['ten']) : '';
		...
		if($d->insert('member',$data))
		{
			...
			$func->transfer("Đăng ký thành viên thành công. Vui lòng đợi quản trị viên kích hoạt tài khoản", $config_base."account/dang-nhap");
		}
		else
		{
